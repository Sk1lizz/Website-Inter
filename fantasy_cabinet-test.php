...
if (isset($_POST['save_squad'])) {
    ...
    if (count($ids) !== 7 || count(array_unique($ids)) !== 7) {
        ...
        if (!$saveError && $oldRow) {
            ...
            // сравниваем старый и новый составы как множества игроков
$oldPlayers = array_values(array_filter($oldIds)); // старые id игроков
$newPlayers = array_values(array_filter($newIds)); // новые id игроков

// Кто появился нового (их и считаем как трансфер)
$added = array_diff($newPlayers, $oldPlayers);

$deltaTransfers = 0;
foreach ($added as $pid) {
    $teamId = 0;
    $qr = $db->query("SELECT team_id FROM players WHERE id=".(int)$pid." LIMIT 1");
    if ($qr && $rr = $qr->fetch_assoc()) {
        $teamId = (int)$rr['team_id'];
    }
...
.sick-badge {
    ...
    width: 22px;
    height: 22px;
    background: url('/img/icon/cross.png') center/contain no-repeat;
    border: none;
    z-index: 3;
    cursor: default;
}
...
var SICK = new Set();
var SICK_URL = 'api/get_sick.php';

function _sickBadgeHTML() {
    return '<span class="sick-badge" title="Игрок болеет"></span>';
}

function markSickInLists() {
    $$('.card').forEach(function(card) {
        var id = parseInt(card.getAttribute('data-id'));
        if (!id) return;
        var ph = card.querySelector('.ph');
        if (!ph) return;
        var old = ph.querySelector('.sick-badge');
        if (old) old.remove();

        if (SICK.has(id)) {
            ph.insertAdjacentHTML('beforeend', _sickBadgeHTML());
        }
    });
}

function markSickOnField() {
    ['gk_id', 'df1_id', 'df2_id', 'mf1_id', 'mf2_id', 'fw_id', 'bench_id'].forEach(function(k) {
        ...
        if (!avatar) return;
        var old = avatar.querySelector('.sick-badge');
        if (old) old.remove();

        if (id && SICK.has(id)) {
            avatar.insertAdjacentHTML('beforeend', _sickBadgeHTML());
        }
    });
}
...
(function(){
  fetch(SICK_URL)
    .then(r=>r.json())
    .then(ids=>{
      SICK=new Set((ids||[]).map(x=>parseInt(x)));
      markSickInLists();
      markSickOnField();
    })
    .catch(err=>console.error('Sick fetch error',err));
})();
