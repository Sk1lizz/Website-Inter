...
if (!isset($input['match_id'], $input['players']) || !is_array($input['players'])) {
...
foreach ($input['players'] as $player_id => $info) {
    ...
    // Обновление статистики (год указываем явно, если нужно для конкретной таблицы)
    $update = $db->prepare("
